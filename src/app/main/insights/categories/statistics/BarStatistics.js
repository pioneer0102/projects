...
const BarStatistics = () => {
    ...
    Cagetories.forEach((channel) => {
        if (Object.entries(saleTotalByCategory).length !== 0)
            saleData.push(saleTotalByCategory[channel]);
        if (Object.entries(orderTotalByCategory).length !== 0)
            orderData.push(orderTotalByCategory[channel]);
    });
